...
setInterval(() => { if (!document.hidden) flushTotals() }, 30000)   // 遊戲中累積的消滅數/遊玩秒數定期上傳（無累積則不送）
document.addEventListener('visibilitychange', () => { if (document.hidden) flushTotals() })   // 離開分頁把累積送出
